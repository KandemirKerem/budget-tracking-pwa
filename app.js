...
let currentExpenseDate = todayStr;
...
document.getElementById('fab-add').addEventListener('click', () => {
    ...
    currentExpenseDate = todayStr;
    openModal('modal-expense');
});

document.getElementById('btn-add-day-expense').addEventListener('click', () => {
    document.getElementById('form-expense').reset();
    currentExpenseDate = window.selectedDayDateStr;
    openModal('modal-expense');
});
...
document.getElementById('form-expense').addEventListener('submit', (e) => {
    ...
    if (amount > 0 && desc) {
        if (!expenses[currentExpenseDate]) expenses[currentExpenseDate] = [];
        expenses[currentExpenseDate].push({ id: Date.now(), amount, desc });
        localStorage.setItem('budget_expenses', JSON.stringify(expenses));

        updateTodayView();
        renderCalendar();

        if (document.getElementById('modal-day-info').classList.contains('active')) {
            const newTotal = expenses[currentExpenseDate].reduce((s, x) => s + x.amount, 0);
            showDayInfo(currentExpenseDate, newTotal);
        }
...
function showDayInfo(dateStr, totalSpent) {
    window.selectedDayDateStr = dateStr;
